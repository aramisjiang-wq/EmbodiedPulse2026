# Robotics ArXiv Daily - 部署方案文档

## 📋 目录

1. [部署架构分析](#部署架构分析)
2. [部署前准备](#部署前准备)
3. [方案一：传统服务器部署（推荐）](#方案一传统服务器部署推荐)
4. [方案二：Docker容器化部署](#方案二docker容器化部署)
5. [方案三：云平台部署](#方案三云平台部署)
6. [数据库迁移与备份](#数据库迁移与备份)
7. [静态资源优化](#静态资源优化)
8. [安全性配置](#安全性配置)
9. [监控与维护](#监控与维护)
10. [故障排查](#故障排查)

---

## 部署架构分析

### 当前技术栈

```
┌─────────────────────────────────────────┐
│           用户浏览器                      │
└──────────────┬──────────────────────────┘
               │ HTTP/HTTPS
               ▼
┌─────────────────────────────────────────┐
│         Nginx (反向代理)                 │
│     - SSL/TLS终止                        │
│     - 静态文件服务                       │
│     - 负载均衡（可选）                   │
└──────────────┬──────────────────────────┘
               │
               ▼
┌─────────────────────────────────────────┐
│      Gunicorn (WSGI服务器)               │
│     - 多进程处理                        │
│     - 进程管理                          │
└──────────────┬──────────────────────────┘
               │
               ▼
┌─────────────────────────────────────────┐
│         Flask应用 (app.py)               │
│     - API路由                           │
│     - 业务逻辑                          │
│     - APScheduler定时任务               │
└──────────────┬──────────────────────────┘
               │
               ▼
┌─────────────────────────────────────────┐
│         PostgreSQL数据库                │
│     - robotics_arxiv (主数据库)         │
│     - papers, jobs, news, datasets表    │
│     - 连接池管理                        │
└─────────────────────────────────────────┘
```

### 组件说明

| 组件 | 作用 | 生产环境要求 |
|------|------|------------|
| **Nginx** | 反向代理、静态文件服务、SSL | 必需 |
| **Gunicorn** | WSGI服务器，多进程处理 | 必需 |
| **Flask** | Web框架，业务逻辑 | 必需 |
| **PostgreSQL** | 数据库（v1.3.0已升级） | 必需 |
| **APScheduler** | 定时任务调度 | 可选 |

---

## 部署前准备

### 1. 代码准备

```bash
# 1. 确保代码已提交到Git仓库
git add .
git commit -m "准备部署"
git push origin main

# 2. 检查关键文件
ls -la app.py models.py requirements.txt config.yaml
ls -la templates/ static/
```

### 2. 环境变量配置

创建 `.env` 文件（**不要提交到Git**）：

```bash
# 数据库配置（PostgreSQL）
DATABASE_URL=postgresql://robotics_user:robotics_password@localhost:5432/robotics_arxiv

# 可选：独立数据库配置
# JOBS_DATABASE_URL=postgresql://...
# NEWS_DATABASE_URL=postgresql://...
# DATASETS_DATABASE_URL=postgresql://...

# 向后兼容SQLite（可选）
# DATABASE_URL=sqlite:///./papers.db

# 服务器配置
PORT=5001
FLASK_ENV=production
DEBUG=false

# 自动抓取配置
AUTO_FETCH_ENABLED=true
AUTO_FETCH_SCHEDULE=0 2 * * *
AUTO_FETCH_MAX_RESULTS=10

# 日志配置
LOG_LEVEL=INFO
```

### 3. 依赖检查

```bash
# 检查requirements.txt是否完整
cat requirements.txt

# 应该包含：
# - flask
# - gunicorn (生产环境必需)
# - sqlalchemy
# - apscheduler
# - arxiv
# - pyyaml
# - requests
```

### 4. 数据库备份

```bash
# 备份现有数据库
cp papers.db papers.db.backup

# 导出数据（可选）
sqlite3 papers.db .dump > papers_backup.sql
```

---

## 方案一：传统服务器部署（推荐）

### 适用场景
- 自有VPS/云服务器
- 需要完全控制
- 成本敏感
- 适合中小型应用

### 服务器要求

**最低配置**：
- CPU: 1核
- 内存: 1GB
- 存储: 10GB
- 带宽: 1Mbps

**推荐配置**：
- CPU: 2核
- 内存: 2GB
- 存储: 20GB SSD
- 带宽: 5Mbps

### 部署步骤

#### 1. 服务器环境准备

```bash
# 更新系统
sudo apt update && sudo apt upgrade -y

# 安装Python 3.9+
sudo apt install python3 python3-pip python3-venv -y

# 安装Nginx
sudo apt install nginx -y

# 安装Git
sudo apt install git -y

# 安装PostgreSQL（推荐）
sudo apt install postgresql postgresql-contrib -y

# 或使用Docker
docker run --name postgres -e POSTGRES_PASSWORD=robotics_password \
  -e POSTGRES_USER=robotics_user -e POSTGRES_DB=robotics_arxiv \
  -p 5432:5432 -d postgres:15-alpine

# 检查PostgreSQL版本
psql --version
```

#### 2. 创建应用用户

```bash
# 创建专用用户（安全考虑）
sudo useradd -m -s /bin/bash robotics-app
sudo su - robotics-app
```

#### 3. 部署代码

```bash
# 克隆代码
cd ~
git clone <your-repo-url> robotics_arxiv_daily
cd robotics_arxiv_daily

# 创建虚拟环境
python3 -m venv venv
source venv/bin/activate

# 安装依赖
pip install --upgrade pip
pip install -r requirements.txt
pip install gunicorn  # 生产环境必需

# 初始化所有数据库（PostgreSQL）
# 确保已配置DATABASE_URL环境变量
python3 init_database.py

# 如果有SQLite数据需要迁移
python3 migrate_sqlite_to_postgresql.py
```

#### 4. 配置Gunicorn

创建 `gunicorn_config.py`：

```python
# gunicorn_config.py
import multiprocessing
import os

# 服务器socket
bind = "127.0.0.1:5001"
backlog = 2048

# Worker进程
workers = multiprocessing.cpu_count() * 2 + 1
worker_class = "sync"
worker_connections = 1000
timeout = 30
keepalive = 2

# 日志
accesslog = "/var/log/robotics-arxiv/access.log"
errorlog = "/var/log/robotics-arxiv/error.log"
loglevel = "info"

# 进程命名
proc_name = "robotics-arxiv"

# 用户和组（如果以root运行）
# user = "robotics-app"
# group = "robotics-app"

# 环境变量
raw_env = [
    "FLASK_ENV=production",
    "AUTO_FETCH_ENABLED=true",
    "AUTO_FETCH_SCHEDULE=0 2 * * *",
]
```

#### 5. 创建Systemd服务

创建 `/etc/systemd/system/robotics-arxiv.service`：

```ini
[Unit]
Description=Robotics ArXiv Daily Web Application
After=network.target

[Service]
Type=notify
User=robotics-app
Group=robotics-app
WorkingDirectory=/home/robotics-app/robotics_arxiv_daily
Environment="PATH=/home/robotics-app/robotics_arxiv_daily/venv/bin"
Environment="DATABASE_URL=postgresql://robotics_user:robotics_password@localhost:5432/robotics_arxiv"
Environment="AUTO_FETCH_ENABLED=true"
Environment="AUTO_FETCH_SCHEDULE=0 2 * * *"
Environment="AUTO_FETCH_MAX_RESULTS=10"
Environment="FLASK_ENV=production"
ExecStart=/home/robotics-app/robotics_arxiv_daily/venv/bin/gunicorn \
    --config /home/robotics-app/robotics_arxiv_daily/gunicorn_config.py \
    app:app
ExecReload=/bin/kill -s HUP $MAINPID
Restart=always
RestartSec=10

[Install]
WantedBy=multi-user.target
```

#### 6. 配置Nginx

创建 `/etc/nginx/sites-available/robotics-arxiv`：

```nginx
server {
    listen 80;
    server_name your-domain.com www.your-domain.com;

    # 重定向到HTTPS（配置SSL后启用）
    # return 301 https://$server_name$request_uri;

    # 日志
    access_log /var/log/nginx/robotics-arxiv-access.log;
    error_log /var/log/nginx/robotics-arxiv-error.log;

    # 静态文件
    location /static/ {
        alias /home/robotics-app/robotics_arxiv_daily/static/;
        expires 30d;
        add_header Cache-Control "public, immutable";
    }

    # 代理到Gunicorn
    location / {
        proxy_pass http://127.0.0.1:5001;
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
        
        # WebSocket支持（如果需要）
        proxy_http_version 1.1;
        proxy_set_header Upgrade $http_upgrade;
        proxy_set_header Connection "upgrade";
        
        # 超时设置
        proxy_connect_timeout 60s;
        proxy_send_timeout 60s;
        proxy_read_timeout 60s;
    }

    # 限制请求大小
    client_max_body_size 10M;
}
```

启用配置：

```bash
sudo ln -s /etc/nginx/sites-available/robotics-arxiv /etc/nginx/sites-enabled/
sudo nginx -t  # 测试配置
sudo systemctl reload nginx
```

#### 7. 配置SSL（Let's Encrypt）

```bash
# 安装Certbot
sudo apt install certbot python3-certbot-nginx -y

# 获取SSL证书
sudo certbot --nginx -d your-domain.com -d www.your-domain.com

# 自动续期（已自动配置）
sudo certbot renew --dry-run
```

#### 8. 启动服务

```bash
# 创建日志目录
sudo mkdir -p /var/log/robotics-arxiv
sudo chown robotics-app:robotics-app /var/log/robotics-arxiv

# 启动服务
sudo systemctl daemon-reload
sudo systemctl start robotics-arxiv
sudo systemctl enable robotics-arxiv

# 检查状态
sudo systemctl status robotics-arxiv
```

#### 9. 防火墙配置

```bash
# 允许HTTP和HTTPS
sudo ufw allow 80/tcp
sudo ufw allow 443/tcp

# 启用防火墙
sudo ufw enable
```

### 维护命令

```bash
# 查看服务状态
sudo systemctl status robotics-arxiv

# 查看日志
sudo journalctl -u robotics-arxiv -f
tail -f /var/log/robotics-arxiv/error.log

# 重启服务
sudo systemctl restart robotics-arxiv

# 更新代码
cd /home/robotics-app/robotics_arxiv_daily
git pull
source venv/bin/activate
pip install -r requirements.txt
sudo systemctl restart robotics-arxiv
```

---

## 方案二：Docker容器化部署

### 适用场景
- 需要快速部署
- 环境一致性要求高
- 微服务架构
- 容器编排（Docker Compose/Kubernetes）

### 1. 创建Dockerfile

```dockerfile
# Dockerfile
FROM python:3.9-slim

# 设置工作目录
WORKDIR /app

# 安装系统依赖
RUN apt-get update && apt-get install -y \
    gcc \
    && rm -rf /var/lib/apt/lists/*

# 复制依赖文件
COPY requirements.txt .

# 安装Python依赖
RUN pip install --no-cache-dir -r requirements.txt
RUN pip install gunicorn

# 复制应用代码
COPY . .

# 初始化数据库（PostgreSQL）
# 注意：需要先配置DATABASE_URL环境变量
RUN python3 init_database.py || true

# 创建非root用户
RUN useradd -m -u 1000 appuser && chown -R appuser:appuser /app
USER appuser

# 暴露端口
EXPOSE 5001

# 健康检查
HEALTHCHECK --interval=30s --timeout=10s --start-period=5s --retries=3 \
    CMD python3 -c "import requests; requests.get('http://localhost:5001/api/stats')"

# 启动命令
CMD ["gunicorn", "-w", "4", "-b", "0.0.0.0:5001", "--access-logfile", "-", "--error-logfile", "-", "app:app"]
```

### 2. 创建docker-compose.yml

```yaml
version: '3.8'

services:
  # PostgreSQL数据库服务
  postgres:
    image: postgres:15-alpine
    container_name: robotics-arxiv-postgres
    environment:
      POSTGRES_USER: robotics_user
      POSTGRES_PASSWORD: robotics_password
      POSTGRES_DB: robotics_arxiv
    volumes:
      - postgres_data:/var/lib/postgresql/data
    ports:
      - "5432:5432"
    restart: always
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U robotics_user -d robotics_arxiv"]
      interval: 10s
      timeout: 5s
      retries: 5

  web:
    build: .
    container_name: robotics-arxiv-web
    ports:
      - "5001:5001"
    volumes:
      # 数据持久化（PostgreSQL使用Docker卷）
      - ./docs:/app/docs
      - ./config.yaml:/app/config.yaml
    environment:
      - DATABASE_URL=postgresql://robotics_user:robotics_password@postgres:5432/robotics_arxiv
      - AUTO_FETCH_ENABLED=true
      - AUTO_FETCH_SCHEDULE=0 2 * * *
      - AUTO_FETCH_MAX_RESULTS=10
      - FLASK_ENV=production
    depends_on:
      postgres:
        condition: service_healthy
    restart: always
    healthcheck:
      test: ["CMD", "python3", "-c", "import requests; requests.get('http://localhost:5001/api/stats', timeout=5)"]
      interval: 30s
      timeout: 10s
      retries: 3
      start_period: 40s

volumes:
  postgres_data:

  nginx:
    image: nginx:alpine
    container_name: robotics-arxiv-nginx
    ports:
      - "80:80"
      - "443:443"
    volumes:
      - ./nginx.conf:/etc/nginx/nginx.conf:ro
      - ./static:/usr/share/nginx/html/static:ro
      - ./ssl:/etc/nginx/ssl:ro
    depends_on:
      - web
    restart: always

volumes:
  postgres_data:
```

### 3. 创建.dockerignore

```
__pycache__
*.pyc
*.pyo
*.pyd
.Python
venv/
env/
*.db
*.db-journal
.git
.gitignore
.DS_Store
*.log
```

### 4. 部署命令

```bash
# 构建镜像
docker-compose build

# 启动服务
docker-compose up -d

# 查看日志
docker-compose logs -f

# 停止服务
docker-compose down

# 更新部署
docker-compose pull
docker-compose up -d --build
```

---

## 方案三：云平台部署

### 3.1 Railway部署

**优点**：简单快速，自动SSL，Git集成

**步骤**：

1. 注册Railway账号
2. 连接GitHub仓库
3. 配置环境变量：
   ```
   AUTO_FETCH_ENABLED=true
   AUTO_FETCH_SCHEDULE=0 2 * * *
   AUTO_FETCH_MAX_RESULTS=10
   ```
4. Railway自动检测并部署

**注意事项**：
- ✅ PostgreSQL已集成（v1.3.0），推荐使用PostgreSQL插件
- 如果使用SQLite，在Railway上可能不稳定
- 需要配置持久化存储

### 3.2 Render部署

**步骤**：

1. 创建 `render.yaml`：

```yaml
services:
  - type: web
    name: robotics-arxiv
    env: python
    buildCommand: pip install -r requirements.txt && python3 init_database.py
    startCommand: gunicorn -w 4 -b 0.0.0.0:$PORT app:app
    envVars:
      - key: AUTO_FETCH_ENABLED
        value: true
      - key: AUTO_FETCH_SCHEDULE
        value: "0 2 * * *"
```

2. 在Render Dashboard连接GitHub仓库
3. 自动部署

### 3.3 Heroku部署

**步骤**：

1. 创建 `Procfile`：

```
web: gunicorn -w 4 -b 0.0.0.0:$PORT app:app
```

2. 创建 `runtime.txt`：

```
python-3.9.16
```

3. 部署：

```bash
heroku create robotics-arxiv-daily
heroku config:set AUTO_FETCH_ENABLED=true
heroku config:set AUTO_FETCH_SCHEDULE="0 2 * * *"
git push heroku main
```

---

## 数据库迁移与备份

### PostgreSQL数据库（v1.3.0已升级）

**当前状态**：所有数据库已升级为PostgreSQL
- ✅ 论文数据库（papers）
- ✅ 招聘信息数据库（jobs）
- ✅ 新闻数据库（news）
- ✅ 数据集数据库（datasets）

### PostgreSQL安装与配置

#### 1. 安装PostgreSQL

```bash
# Ubuntu/Debian
sudo apt install postgresql postgresql-contrib -y

# macOS
brew install postgresql
brew services start postgresql

# 或使用Docker
docker run --name postgres -e POSTGRES_PASSWORD=robotics_password \
  -e POSTGRES_USER=robotics_user -e POSTGRES_DB=robotics_arxiv \
  -p 5432:5432 -d postgres:15-alpine
```

#### 2. 创建数据库和用户

```sql
-- 连接到PostgreSQL
psql -U postgres

-- 创建数据库
CREATE DATABASE robotics_arxiv;

-- 创建用户
CREATE USER robotics_user WITH PASSWORD 'robotics_password';

-- 授予权限
GRANT ALL PRIVILEGES ON DATABASE robotics_arxiv TO robotics_user;

-- 退出
\q
```

#### 3. 环境变量配置

```bash
# 主数据库
export DATABASE_URL=postgresql://robotics_user:robotics_password@localhost:5432/robotics_arxiv

# 可选：独立数据库配置
export JOBS_DATABASE_URL=postgresql://...
export NEWS_DATABASE_URL=postgresql://...
export DATASETS_DATABASE_URL=postgresql://...
```

#### 4. 初始化数据库

```bash
# 初始化所有数据库表
python3 init_database.py
```

### SQLite → PostgreSQL数据迁移

如果已有SQLite数据，使用自动迁移脚本：

```bash
# 运行迁移脚本
python3 migrate_sqlite_to_postgresql.py
```

**迁移脚本功能**：
- ✅ 自动检测PostgreSQL连接
- ✅ 迁移所有4个数据库（papers, jobs, news, datasets）
- ✅ 自动跳过已存在的记录
- ✅ 显示迁移统计信息
- ✅ 完善的错误处理

**手动迁移（高级）**：

```python
# 使用pandas迁移（示例）
from sqlalchemy import create_engine
import pandas as pd

# SQLite连接
sqlite_engine = create_engine('sqlite:///papers.db')

# PostgreSQL连接
pg_engine = create_engine('postgresql://user:pass@localhost/robotics_arxiv')

# 读取SQLite数据
df = pd.read_sql('SELECT * FROM papers', sqlite_engine)

# 写入PostgreSQL
df.to_sql('papers', pg_engine, if_exists='append', index=False)
```

### PostgreSQL自动备份脚本

创建 `backup_postgresql.sh`：

```bash
#!/bin/bash
BACKUP_DIR="/backup/robotics-arxiv"
DATE=$(date +%Y%m%d_%H%M%S)
DB_NAME="robotics_arxiv"
DB_USER="robotics_user"

mkdir -p $BACKUP_DIR

# PostgreSQL备份
pg_dump -U $DB_USER -d $DB_NAME -F c -f $BACKUP_DIR/robotics_arxiv_$DATE.dump

# 压缩备份
gzip $BACKUP_DIR/robotics_arxiv_$DATE.dump

# 删除7天前的备份
find $BACKUP_DIR -name "*.dump.gz" -mtime +7 -delete

echo "备份完成: robotics_arxiv_$DATE.dump.gz"
```

**恢复备份**：

```bash
# 解压
gunzip robotics_arxiv_20251211_120000.dump.gz

# 恢复
pg_restore -U robotics_user -d robotics_arxiv -c robotics_arxiv_20251211_120000.dump
```

**SQLite备份（向后兼容）**：

```bash
#!/bin/bash
BACKUP_DIR="/backup/robotics-arxiv"
DATE=$(date +%Y%m%d_%H%M%S)

mkdir -p $BACKUP_DIR

# 备份SQLite数据库
cp papers.db $BACKUP_DIR/papers_$DATE.db

# 压缩备份
gzip $BACKUP_DIR/papers_$DATE.db

# 删除7天前的备份
find $BACKUP_DIR -name "*.db.gz" -mtime +7 -delete

echo "备份完成: papers_$DATE.db.gz"
```

添加到Cron：

```bash
# 每天凌晨3点备份
0 3 * * * /path/to/backup.sh
```

---

## 静态资源优化

### 1. CDN配置

将静态资源（CSS/JS/图片）上传到CDN：

```html
<!-- 使用CDN -->
<link rel="stylesheet" href="https://cdn.example.com/static/css/style.css">
<script src="https://cdn.example.com/static/js/app.js"></script>
```

### 2. 压缩优化

```bash
# 压缩CSS/JS
npm install -g clean-css-cli uglify-js
cleancss -o static/css/style.min.css static/css/style.css
uglifyjs static/js/app.js -o static/js/app.min.js
```

### 3. 浏览器缓存

Nginx配置已包含缓存设置（30天）。

---

## 安全性配置

### 1. 环境变量保护

```bash
# 不要将敏感信息提交到Git
echo ".env" >> .gitignore
echo "*.db" >> .gitignore
```

### 2. Flask安全配置

在 `app.py` 中添加：

```python
app.config['SECRET_KEY'] = os.getenv('SECRET_KEY', 'change-this-in-production')
app.config['SESSION_COOKIE_SECURE'] = True  # HTTPS only
app.config['SESSION_COOKIE_HTTPONLY'] = True
app.config['SESSION_COOKIE_SAMESITE'] = 'Lax'
```

### 3. 防火墙规则

```bash
# 只允许必要端口
sudo ufw allow 22/tcp   # SSH
sudo ufw allow 80/tcp   # HTTP
sudo ufw allow 443/tcp # HTTPS
sudo ufw allow 5432/tcp # PostgreSQL（如需要外部访问）
sudo ufw enable
```

### 4. 定期更新

```bash
# 系统更新
sudo apt update && sudo apt upgrade -y

# Python包更新
pip list --outdated
pip install --upgrade <package>
```

---

## 监控与维护

### 1. 日志监控

```bash
# 实时查看日志
tail -f /var/log/robotics-arxiv/error.log

# 查看访问日志
tail -f /var/log/robotics-arxiv/access.log
```

### 2. 性能监控

安装监控工具：

```bash
pip install prometheus-client
```

### 3. 健康检查

创建健康检查端点：

```python
@app.route('/health')
def health():
    try:
        session = get_session()
        session.query(Paper).limit(1).all()
        session.close()
        return jsonify({'status': 'healthy'}), 200
    except:
        return jsonify({'status': 'unhealthy'}), 500
```

### 4. 告警配置

使用监控服务（如UptimeRobot）定期检查 `/health` 端点。

---

## 故障排查

### 常见问题

#### 1. 服务无法启动

```bash
# 检查日志
sudo journalctl -u robotics-arxiv -n 50

# 检查端口占用
sudo netstat -tlnp | grep 5001

# 检查权限
ls -la papers.db
```

#### 2. 数据库锁定（已解决）

```bash
# v1.3.0已升级到PostgreSQL，支持高并发
# 如果仍使用SQLite遇到锁定问题：
# 1. 关闭所有数据库连接，或重启服务
# 2. 建议升级到PostgreSQL：使用 migrate_sqlite_to_postgresql.py
```

#### 3. 定时任务不执行

```bash
# 检查APScheduler日志
grep -i scheduler /var/log/robotics-arxiv/error.log

# 检查环境变量
echo $AUTO_FETCH_ENABLED
```

#### 4. 内存不足

```bash
# 减少Gunicorn worker数量
workers = 2  # 在gunicorn_config.py中
```

---

## 部署检查清单

- [ ] 代码已提交到Git
- [ ] PostgreSQL已安装并运行（v1.3.0）
- [ ] 数据库和用户已创建
- [ ] 环境变量已配置（DATABASE_URL等）
- [ ] 所有数据库已初始化（`python3 init_database.py`）
- [ ] 如有SQLite数据，已迁移到PostgreSQL（`python3 migrate_sqlite_to_postgresql.py`）
- [ ] Gunicorn已安装
- [ ] Systemd服务已配置
- [ ] Nginx已配置
- [ ] SSL证书已配置
- [ ] 防火墙已配置（开放5432端口，如需要）
- [ ] PostgreSQL备份脚本已设置
- [ ] 监控已配置
- [ ] 日志目录已创建
- [ ] 服务已启动并测试
- [ ] 数据库连接测试通过（`python3 tests/test_database_connections.py`）

---

**文档创建时间**: 2025-12-08  
**最后更新时间**: 2025-12-11  
**文档维护者**: 项目开发团队  
**更新内容**: v1.3.0 - PostgreSQL数据库全面升级、Docker Compose集成、数据迁移工具


