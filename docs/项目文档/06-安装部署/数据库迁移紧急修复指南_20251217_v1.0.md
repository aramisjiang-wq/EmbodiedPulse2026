# 数据库迁移紧急修复指南

**版本**: v1.0  
**日期**: 2025-12-17  
**问题**: 服务器数据库文件存在但数据为空（记录数为0）

---

## 📋 问题诊断

### 当前状态

**服务器数据库状态**:
- ✅ 论文数据库文件存在: `/srv/EmbodiedPulse2026/instance/papers.db` (68K)
- ✅ B站数据库文件存在: `/srv/EmbodiedPulse2026/bilibili.db` (432K)
- ❌ 论文数量: 0
- ❌ UP主数量: 0
- ❌ 视频数量: 0

**本地数据库状态**:
- ✅ B站数据库有数据: 12个UP主，516个视频
- ⚠️ 论文数据库只有认证表，没有papers表（需要重新抓取或从JSON迁移）

---

## 🔧 解决方案

### 方案1：从本地复制B站数据库（推荐）

**B站数据库有数据，可以直接复制**

#### 步骤1：在本地备份数据库

```bash
cd "/Users/dong/Documents/Cursor/Embodied Pulse"

# 备份B站数据库
cp bilibili.db backups/bilibili_backup_$(date +%Y%m%d_%H%M%S).db

# 验证备份
ls -lh backups/bilibili_backup_*.db
```

#### 步骤2：上传到服务器

**方法1：使用 scp（推荐）**

```bash
# 在本地执行
scp bilibili.db root@101.200.222.139:/srv/EmbodiedPulse2026/

# 输入密码: XLj4kUnh
```

**方法2：使用迁移脚本**

```bash
# 在本地执行
cd "/Users/dong/Documents/Cursor/Embodied Pulse"
bash scripts/migrate_database_to_server.sh
```

#### 步骤3：在服务器上设置权限

```bash
# SSH登录服务器
ssh root@101.200.222.139

# 进入项目目录
cd /srv/EmbodiedPulse2026

# 设置文件权限
chmod 644 bilibili.db
chown root:root bilibili.db

# 验证数据
sqlite3 bilibili.db "SELECT COUNT(*) FROM bilibili_ups;"
sqlite3 bilibili.db "SELECT COUNT(*) FROM bilibili_videos;"

# 应该显示：
# 12 (UP主数量)
# 516 (视频数量)
```

#### 步骤4：重启服务

```bash
# 重启服务使数据库生效
systemctl restart embodiedpulse

# 检查服务状态
systemctl status embodiedpulse
```

---

### 方案2：论文数据库处理

**论文数据库当前只有认证表，没有papers表**

#### 选项A：让系统自动抓取（推荐）

论文数据可以通过定时任务自动抓取，无需手动迁移。

**检查定时任务**:
```bash
# 在服务器上执行
cd /srv/EmbodiedPulse2026

# 检查环境变量
cat .env | grep AUTO_FETCH_ENABLED

# 应该显示: AUTO_FETCH_ENABLED=true

# 检查定时任务是否运行
journalctl -u embodiedpulse | grep -i "定时任务\|scheduler"

# 应该看到: "定时任务调度器已启动"
```

**手动触发抓取**:
```bash
# 在服务器上执行
cd /srv/EmbodiedPulse2026
source venv/bin/activate

# 手动触发论文抓取
python3 -c "from fetch_new_data import fetch_papers; fetch_papers()"
```

#### 选项B：从JSON文件迁移（如果有历史数据）

如果本地有JSON格式的论文数据，可以迁移：

```bash
# 在服务器上执行
cd /srv/EmbodiedPulse2026
source venv/bin/activate

# 运行迁移脚本（如果有）
python3 migrate_json_to_db.py
```

---

## 📝 完整迁移步骤（一键执行）

### 在本地执行

```bash
cd "/Users/dong/Documents/Cursor/Embodied Pulse"

# 1. 备份数据库
mkdir -p backups
cp bilibili.db backups/bilibili_backup_$(date +%Y%m%d_%H%M%S).db

# 2. 上传B站数据库到服务器
scp bilibili.db root@101.200.222.139:/srv/EmbodiedPulse2026/

# 输入密码: XLj4kUnh
```

### 在服务器上执行

```bash
# SSH登录服务器
ssh root@101.200.222.139

# 进入项目目录
cd /srv/EmbodiedPulse2026

# 设置文件权限
chmod 644 bilibili.db
chown root:root bilibili.db

# 验证数据
sqlite3 bilibili.db "SELECT COUNT(*) FROM bilibili_ups;"
sqlite3 bilibili.db "SELECT COUNT(*) FROM bilibili_videos;"

# 重启服务
systemctl restart embodiedpulse

# 检查服务状态
systemctl status embodiedpulse

# 验证网站数据
# 访问 https://blibli.gradmotion.com 检查数据是否正常显示
```

---

## ✅ 验证步骤

### 1. 验证B站数据库

```bash
# 在服务器上执行
cd /srv/EmbodiedPulse2026

# 检查UP主数量
sqlite3 bilibili.db "SELECT COUNT(*) FROM bilibili_ups;"
# 应该显示: 12

# 检查视频数量
sqlite3 bilibili.db "SELECT COUNT(*) FROM bilibili_videos;"
# 应该显示: 516

# 查看前3个UP主
sqlite3 bilibili.db "SELECT name, fans_formatted FROM bilibili_ups LIMIT 3;"
```

### 2. 验证论文数据库

```bash
# 在服务器上执行
cd /srv/EmbodiedPulse2026

# 检查是否有papers表
sqlite3 instance/papers.db ".tables"

# 如果有papers表，检查数量
sqlite3 instance/papers.db "SELECT COUNT(*) FROM papers;" 2>/dev/null || echo "papers表不存在"

# 如果没有papers表，等待定时任务抓取或手动触发
```

### 3. 验证网站显示

1. **访问B站页面**: https://blibli.gradmotion.com
   - 应该能看到UP主列表
   - 应该能看到视频数据

2. **访问论文页面**: https://essay.gradmotion.com
   - 如果papers表不存在，页面可能显示"暂无数据"
   - 等待定时任务抓取或手动触发抓取

---

## 🚨 常见问题

### 问题1：上传后数据仍然是0

**可能原因**:
- 文件权限错误
- 服务未重启
- 数据库文件损坏

**解决方法**:
```bash
# 检查文件权限
ls -l /srv/EmbodiedPulse2026/bilibili.db
# 应该是: -rw-r--r-- 1 root root

# 检查文件大小
ls -lh /srv/EmbodiedPulse2026/bilibili.db
# 应该大于 400K

# 重启服务
systemctl restart embodiedpulse

# 验证数据
sqlite3 /srv/EmbodiedPulse2026/bilibili.db "SELECT COUNT(*) FROM bilibili_ups;"
```

### 问题2：论文数据库没有papers表

**这是正常的**，如果之前没有抓取过论文数据。

**解决方法**:
1. 等待定时任务自动抓取（每小时）
2. 或手动触发抓取（见上方"方案2"）

### 问题3：表名不匹配

**B站数据库表名**:
- `bilibili_ups`（复数，不是 `bilibili_up`）
- `bilibili_videos`（复数，不是 `bilibili_video`）

**检查脚本已更新**，使用正确的表名。

---

## 📊 迁移后预期结果

### B站数据库
- ✅ UP主数量: 12
- ✅ 视频数量: 516
- ✅ 网站正常显示数据

### 论文数据库
- ⚠️ 论文数量: 0（初始状态，等待抓取）
- ✅ 认证表正常（auth_users, admin_users等）
- ✅ 定时任务会自动抓取论文数据

---

## 🔄 后续操作

### 1. 监控数据更新

```bash
# 检查定时任务是否运行
journalctl -u embodiedpulse | grep -i "定时任务"

# 检查论文抓取进度
journalctl -u embodiedpulse | grep -i "论文\|paper"

# 检查B站数据更新
journalctl -u embodiedpulse | grep -i "bilibili\|B站"
```

### 2. 手动触发数据抓取

```bash
# 在服务器上执行
cd /srv/EmbodiedPulse2026
source venv/bin/activate

# 触发论文抓取
python3 -c "from fetch_new_data import fetch_papers; fetch_papers()"

# 触发B站数据抓取
python3 -c "from fetch_bilibili_data import fetch_and_save_up_data; fetch_and_save_up_data()"
```

---

**文档版本**: v1.0  
**最后更新**: 2025-12-17  
**维护者**: AI Assistant

