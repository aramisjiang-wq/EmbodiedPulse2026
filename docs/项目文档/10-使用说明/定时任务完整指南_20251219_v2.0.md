...
- [ ] **PostgreSQL数据库连接正常**
  ```bash
  # 检查PostgreSQL连接
  python3 -c "from models import get_session; session = get_session(); print('✅ 数据库连接正常'); session.close()"
  ```
...
```bash
# 使用PostgreSQL查询
python3 -c "from models import get_session, Paper; session = get_session(); print(f'论文数量: {session.query(Paper).count()}'); session.close()"
```

**查看视频数据**：
```bash
# 使用PostgreSQL查询
python3 -c "from bilibili_models import get_bilibili_session, BilibiliUp; session = get_bilibili_session(); print(f'UP主数量: {session.query(BilibiliUp).count()}'); session.close()"
```
