...
def start_scheduler():
    ...
    # 检查是否启用
    if not os.getenv('AUTO_FETCH_ENABLED', 'false').lower() == 'true':
        return
    ...
    def scheduled_fetch():
        """定时抓取任务"""
        if fetch_status['running']:
            logger.info("定时任务跳过：已有任务运行")
            return
        
        try:
            logger.info("开始执行定时抓取任务...")
            config = load_config('config.yaml')
            config['max_results'] = int(os.getenv('AUTO_FETCH_MAX_RESULTS', 10))
            demo(**config)
    ...
    # 配置Cron表达式
    schedule = os.getenv('AUTO_FETCH_SCHEDULE', '0 2 * * *')
    ...
    if len(parts) == 5:
        ...
        scheduler.start()
        logger.info(f"定时任务已启动: {schedule}")
```
...
AUTO_FETCH_SCHEDULE="0 2 * * *"   # Cron表达式
AUTO_FETCH_MAX_RESULTS=10          # 每次抓取数量
