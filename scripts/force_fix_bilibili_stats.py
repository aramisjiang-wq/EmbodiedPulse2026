...
# -*- coding: utf-8 -*-
"""
强制修复Bilibili UP主统计数据
从视频表重新计算videos_count和views_count，强制更新
"""
import os
import sys
...
def force_fix_up_stats():
    """强制修复UP主统计数据"""
    print("=" * 60)
    print("强制修复Bilibili UP主统计数据")
    print("=" * 60)
    ...
    try:
        ...
        for up in ups:
            ...
            if old_videos_count != video_count or old_views_count != total_views:
                ...
                print(f"✓  {up.name} (UID: {up.uid}) - 数据已正确")
        
        # 提交更改
        session.commit()
        print(f"\n✅ 成功更新 {len(ups)} 个UP主的统计数据")
